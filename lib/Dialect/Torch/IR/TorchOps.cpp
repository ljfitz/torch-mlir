...
void PrimUninitializedOp::getCanonicalizationPatterns(RewritePatternSet &patterns,
                                                    MLIRContext *context) {
...
OpFoldResult AtenNegIntOp::fold(ArrayRef<Attribute> operands) {
  int64_t c;
  bool constant = matchPattern(getOperand(), m_TorchConstantInt(&c));
  if (constant)
